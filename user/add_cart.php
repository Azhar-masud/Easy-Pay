...
?>
<?php 
#querry for insert data in add cart
$select_pdt_add_cart_all_row=mysql_query("SELECT * FROM product WHERE pro_id='$id'");
$select_col_in_pdt_add_cart_col=mysql_fetch_array($select_pdt_add_cart_all_row);
if(isset($_POST['add_cart']))
{
	$select_quantity=$_POST['quantity'];
	$pdt_name=$select_col_in_pdt_add_cart_col[2];
	$pdt_price=$select_col_in_pdt_add_cart_col[5];
	$pdt_total_price=$select_quantity*$pdt_price;
	$pdt_detailes=$select_col_in_pdt_add_cart_col[6];
	$item_name=$select_col_in_pdt_add_cart_col[0];
	if($_SESSION['loginfo']==true)
	{
		#query add_cart_table_for_ck_product
		$select_add_cart=mysql_query("SELECT * FROM add_card_data WHERE session_id='$s_id' AND pdt_id='$id' AND email='$email'");
		$select_add_cart_all_data=mysql_fetch_array($select_add_cart);
		if($id != $select_add_cart_all_data[2])
		{


			mysql_query("INSERT INTO add_card_data(`session_id`,`email`,`pdt_id`,`pdt_name`,`pdt_price`,`pdt_quantity`,`total_price`,`detailes`,`item_name`)VALUES('$s_id','$email','$id','$pdt_name','$pdt_price','$select_quantity','$pdt_total_price','$pdt_detailes','$item_name')");
			if(mysql_affected_rows()>0)
			{
				echo "<script>alert('This product added successfully')</script>";
			}
			else
			{
				echo "<script>alert('This product added is not successfully')</script>";
			}
		}
		else
		{
...
{
	echo "<script>location='easy_main.php?&&page=shopping_cart'</script>";
}
				...
				<div class="row">	
			    	<div class="col-md-6">
			    		<div class="row">
			    			...
			    			<div class="col-md-8">
			    				<button class="btn btn-success form-control" name="add_cart" type="submit">Add cart</button>
			    			</div>
			    			<div class="col-md-2"></div>
			    		</div>
			    	</div>
			    	<div class="col-md-6">
			    		<div class="row">
			    			<div class="col-md-2">	</div>
			    			<div class="col-md-8">
			    				<button class="btn btn-success form-control" name="shopping_cart" type="submit">Shopping cart</button>
			    			</div>
			    			<div class="col-md-2"></div>
			    		</div>
			    	</div>
